...
class UrlbackHandler(JSONBaseHandler):
    def get(self, platform):
        log_path = self.config['log']['selfurl_log']
        utils.print_log('selfurl', log_path, self.request.uri)
        if platform == 'adcolony':
            self.__save_adcolony_aos_order()

    def __save_adcolony_aos_order(self):
        self.return_success()

class OauthHandler(JSONBaseHandler):
    def get(self, platform):
        log_path = self.config['log']['selfurl_log']
        utils.print_log('selfurl', log_path, self.request.uri)
        if platform == 'Oauth':
            self.__save_Oauth_aos_order()

    def __save_Oauth_aos_order(self):
        self.return_success()

class AdyouMHandler(JSONBaseHandler):
    def get(self, platform):

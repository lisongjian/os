...
class FeedbackHandler(protocols.JSONBaseHandler):
    ...
    def post(self):
        ...
        if  content:
            self.db.execute(
                "INSERT INTO `feedback`(`scopeid`, `content`,`mail`)"
                "VALUES(%s, %s, %s)", scopeid, content, mail)
            self.return_success()
        else:
    ...
    def get(self):
        ...
        if (share['share_fb']==0 and type=="1") or (share['share_tw']==0 and type=="2") or (share['share_gg']==0 and type=="3"):
            ...
            self.db.execute(
                ...
                "VALUES(%s,%s,%s,%s,%s,%s)", order,ad,self.current_user['uid'],3,2,prize)
            self.db.execute(
            ...
            self.db.execute(
                "INSERT INTO `callback_order`(`order`,`ad`,`user`,`platform`,`ad_source`,`points`)"
                "VALUES(%s,%s,%s,%s,%s,%s)", order,ad,self.current_user['uid'],3,8,prize)
            self.db.execute(
                "INSERT INTO `global_orders` (uid, points, last, type, note)"
                "VALUES (%s, %s, %s, %s, %s)",
                self.current_user['uid'], prize, self.current_user['points'], 8, ad)

...
class NewOrderHandler(protocols.JSONBaseHandler):
    ...
    def __log(self,oid,user,scopeid,id,goods_id,type_id,title,num,pwd,inprice,outprice,orderid):
        ...
        if not self.autologger:
            ...
            self.autologger.info("oid="+str(oid)+"&user="+str(user)+"&scopeid="+scopeid+
...
class ListGoodsHandler(protocols.JSONBaseHandler):
    ...
    def get(self):
        ...
        code = int(self.arguments.get('code', 0))
        channel = self.arguments.get('channel', '')
        if code ==6:
            types = self.db.query("SELECT * FROM `exchange_types` WHERE `tid`=%s AND `code`=%s AND `channel`!=%s", tid, code, channel)
        else :
            types = self.db.query("SELECT * FROM `exchange_types` WHERE `tid`=%s AND `code`!=6 AND `channel`!=%s", tid, channel)
        ...
        for item in orders:
            if item['goods_id']==64 or item['goods_id']==65 or item['goods_id']==66 \
                ...
                formated_item = {
                    ...
                    'uid': item['num'],
                    'end_date': item['end_date'] if item['end_date'] else 0,
                }

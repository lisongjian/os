...
class AdyoumiHandler(JSONBaseHandler):
    ...
    def __save_fyber_order(self):
        params = {}
        for key in ['ip', 'subid', 'google_ad_id', 'device_id']:
            params[key] = self.get_argument(key, "")
        ad_info = self.db.get(
            "SELECT * FROM `wallad_fyber` WHERE `device_id`=%s LIMIT 1", params['device_id'])
        if not ad_info:
            self.db.execute(
                "INSERT INTO `wallad_fyber` (ip, subid, google_ad_id, create_time, device_id)"
                "VALUES (%s, %s, %s, %s, %s)",
                params['ip'], params['subid'], params['google_ad_id'], \
                datetime.datetime.now(), params['device_id'])
        url = 'https://play.google.com/store/apps/details?id=net.ym.overseas.cashme&referrer=utm_source%3Dfyber%26utm_medium%3Dofferwall%26utm_campaign%3Dcpi'
        self.redirect(url)

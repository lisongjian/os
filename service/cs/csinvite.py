...
class CheckInviteHandler(protocols.JSONBaseHandler):
    ...
    def get(self):
        ...
        user=self.db.get(
                    ...
                    WHERE `invited_code` = %s",
                    invitecode)
        if user:
            user=self.db.get(
                "SELECT * FROM `user` WHERE `invited_code` = %s AND `scopeid` = %s",
        ...
        InviteCheat = self.db.get(
            ...
            user['uid'])
        utils.print_log('invite', log_path, self.request.uri+"&byInviteUid="+str(user['uid'])+"&InviteUid="+str(self.current_user['uid']))
        ...
        if InviteCheat:
            if InviteCheat['count(*)']<5:
                ...
                if totalInvite['count(*)'] < maxInvite['values']:
                   # 给邀请人加积分
                    ...
                    self.db.execute(
                        ...
                        "VALUES(%s,%s,%s,%s,%s,%s)", order,ad,user['uid'],3,3,prize)
                    self.db.execute(
                ...
                self.db.execute(
                    "INSERT INTO `callback_order`(`order`,`ad`,`user`,`platform`,`ad_source`,`points`)"
                    "VALUES(%s,%s,%s,%s,%s,%s)", order,ad,self.current_user['uid'],3,3,prize)
                self.db.execute(
                    "INSERT INTO `global_orders` (uid, points, last, type, note)"
                    "VALUES (%s, %s, %s, %s, %s)",

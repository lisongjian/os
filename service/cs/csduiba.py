...
class NotifyHandler(protocols.JSONBaseHandler):
    ...
    def get(self):
        ...
        order = self.db.get(
                "SELECT * FROM `exchange_orders` WHERE `oid`= %s ", self.params['bizId'])
        self.user_info = self.db.get(
            "SELECT * FROM `user` WHERE `uid` = %s",
            order['uid'])
       # 订单不存在
        ...
        if self.params['success'].lower() == 'false':
            self.set_order_status(self.params['bizId'],
            ...
            self.set_order_status(self.params['bizId'], 1, notes)
    ...
    def set_order_status(self, oid, status, notes):
        return self.db.execute(

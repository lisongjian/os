...
class CallbackHandler(JSONBaseHandler):
    ...
    def get(self, platform):
        ...
        if platform == 'tapjoyaos' :
            ...
            self.write('ok')
            self.__log(self.ip)
        elif platform == 'kakayoumi':
            self.__save_kakayoumi_order()
            self.write('ok')
            self.__log(self.ip)
        elif platform == 'kakaduomeng':
            self.__save_kakaduomeng_order()
            self.write('ok')
            self.__log(self.ip)
    ...
    def __save_supersonicadsios_order(self):
        ...
        uid=self.db.get(
            "SELECT `uid` FROM `user` WHERE `scopeid`=%s",
            params['scopeid'])
        self.ip = str(str( self.request.headers.get('X-Real-Ip', None)).split(',')[0])
        ip_info = self.db.get("SELECT * FROM `ip_blacklist` WHERE `ip` = %s AND `status`=1", self.ip)
        black_user = self.db.get("SELECT * FROM `user_blacklist` WHERE `uid` = %s AND `status`=1", uid['uid'])
    ...
    def __save_youmi_order(self):
        params = {}
        self.ip = str( self.request.headers.get('X-Real-Ip', None)).split(',')[0]
        if self.ip == '58.63.244.57' or self.ip =='58.63.244.58':
            pass
        else:
            self.ip = "&ERRip="+str(self.ip)
            return
    ...
    def __save_appdriveros_order(self):
        ...
        ip_info = self.db.get("SELECT * FROM `ip_blacklist` WHERE `ip` = %s AND `status`=1", self.ip)
        params['ad'] = "appdriver紅利"
        params['getPoint'] = self.get_argument('payment', "")
    ...
    def __save_kakaduomeng_order(self):
        params = {}
        self.ip = str( self.request.headers.get('X-Real-Ip', None)).split(',')[0]
        ...
        #self.valild_str =  hashlib.md5(valild).hexdigest()
        uid=self.db.get(
            "SELECT `uid` FROM `user` WHERE `scopeid`=%s and `appType`=1",
            params['user'])
        if uid==None:
            return
        ...
        orderinfo = self.db.get(
            "SELECT * FROM `callback_order` WHERE `ad_source`=201 AND `adid`=%s AND `user`=%s LIMIT 1", params['orderid'], uid['uid'])
        if not orderinfo:
            self.saveOrder(
                uid['uid'], params['order'], params['order'], params['orderid'], params['user'], params['channel'],
                params['point'], params['price'], params['ts'], params['device'], verifier, 201, 3, detail)
            self.push(params['user'],params['order'],params['point'])

    def __save_kakayoumi_order(self):
        params = {}
        self.ip = str( self.request.headers.get('X-Real-Ip', None)).split(',')[0]
        ...
        if black_user:
            ...
            return
        info = self.db.get(
            "SELECT * FROM `callback_order` WHERE `user`=%s and `order`=%s and `ad_source`=200 LIMIT 1",
            uid['uid'],params['order'])
        if not info:
            self.saveOrder(
                uid['uid'], params['order'], params['ad'], params['adid'], params['scopeid'], params['chn'],
                params['getPoint'], params['price'], 0, params['device'], params['sig'], 200, 3, detail)
            self.push(params['scopeid'],params['ad'],params['getPoint'])
    ...
    def __save_adxmi_order(self):
        ...
        for k in kv:
            ...
            # 排序
        if  utils.md5_adxmisign(self.params) != params['sign']:
        ...
        detail = " aos ( 卡卡应用名:"+self.get_argument('ad', '')+" |包名:"+params['pkg']+ \
    ...
    def saveOrder(
        ...
        user_info = self.db.get(
            "SELECT * FROM `user` WHERE `uid` = %s and `appType`=1", uid)
        ...
        self.db.execute(
            ...
            `sig`, `platform`, `ad_source`, `appType`) '
        ...
        self.db.execute(
            ...
            "WHERE `user`.`uid` = %s AND `appType`=1", points, points, uid)
